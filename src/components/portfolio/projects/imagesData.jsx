import { tanLogo, circleImg, pgb, blueYellow, purpleArrow, colourful, dots, noBGLogoPurple, onSite, roadntyre, shipping, staryNight, tornSpace, diploma, sdg, jsSololearn, javaIntroSololearn, gads, azure, gpHero, gpMobile, construction, googleSignIn, addTravelDeal, calender, clock, contactPage, notes, signup, login, travel } from '../imagesExports'

const websiteGallery = [

    {
        galleryImage: gpHero,
        alt: 'golden pride hero'
    },

    {
        galleryImage: contactPage,
        alt: 'Contact page screenshot'
    },

    {
        galleryImage: construction,
        alt: 'construction site'
    },
]

const repoFilesGallery = [


    {
        galleryImage: gpMobile,
        alt: 'golden pride mobile hero'
    },

    {
        galleryImage: googleSignIn,
        alt: 'google sign in screenshot'
    },
    {
        galleryImage: addTravelDeal,
        alt: 'add travel deal screenshot'
    },
    {
        galleryImage: calender,
        alt: 'calender screenshot'
    },
    {
        galleryImage: clock,
        alt: 'clock screenshot'
    },

    {
        galleryImage: notes,
        alt: 'notes screenshot'
    },
    {
        galleryImage: signup,
        alt: 'Sign up screenshot'
    },
    {
        galleryImage: login,
        alt: 'login screenshot'
    },
    {
        galleryImage: travel,
        alt: 'Travelmantics screenshot'
    },
]

const certificatesGallery = [

    {
        galleryImage: diploma,
        alt: 'Tanaka IT diploma'
    },
    {
        galleryImage: sdg,
        alt: 'SDG certificate'
    },
    {
        galleryImage: gads,
        alt: 'GADS certificate'
    },
    {
        galleryImage: azure,
        alt: 'Microsoft azure certificate'
    },
    {
        galleryImage: javaIntroSololearn,
        alt: 'Java introduction sololearn certificate'
    },
    {
        galleryImage: jsSololearn,
        alt: 'JavaScript introduction sololearn certificate'
    },
]


const graphicsGallery = [


    {
        galleryImage: tornSpace,
        alt: 'space image'
    },
    {
        galleryImage: circleImg,
        alt: 'circle Logo'
    },
    {
        galleryImage: pgb,
        alt: 'pink green and blue image'
    },
    {
        galleryImage: staryNight,
        alt: 'Tanaka staryNight image'
    },
    {
        galleryImage: blueYellow,
        alt: 'blue and yellow image'
    },
    {
        galleryImage: purpleArrow,
        alt: 'purple arrow'
    },

    {
        galleryImage: colourful,
        alt: 'colourful image'
    },
    {
        galleryImage: dots,
        alt: 'image of warped dots'
    },
    {
        galleryImage: noBGLogoPurple,
        alt: 'no bg logo'
    },
    {
        galleryImage: onSite,
        alt: 'onsite image'
    },
    {
        galleryImage: roadntyre,
        alt: 'road and tyre pamphlet'
    },
    {
        galleryImage: shipping,
        alt: 'shipping pamphlet'
    },

    {
        galleryImage: tanLogo,
        alt: 'Tanaka Logo'
    },


]




export { graphicsGallery, certificatesGallery, repoFilesGallery, websiteGallery };